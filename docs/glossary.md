ZaaS terminology consolidates the terms for all the ZaaS offerings in alphabetical order to help both customers and internal team members to understand our offerings better.
...
## Cluster/replica set
A node cluster across Availability Zones provisioned by every Hyper Protect DBaaS service instance. Each cluster consists of three nodes that implement replication and automated failover. 
...
## Genesis
Refer to VPC Gen 2. Also called Next Gen.
...
## IKS
The abbreviation of IBM Cloud Kubernetes Service, which is a managed offering to create your own Kubernetes cluster of compute hosts to deploy and manage containerized apps on IBM Cloud.

## IKS Carrier
The kubernetes cluster used by IKS to provide the managed element of their offering. Essentially the customers masters from their kubernetes clusters are actually pods running within this management carrier cluster.

## IKS Cruiser
A specific instance of the managed IKS service for a particular customer.

## IKS with Hyper Protect
The ZaaS Containers/Kubernetes offering. ZaaS offerings in public cloud which take advantage of IKS existing cloud offerings to provide a Hyper Protect option for the kubernetes workers/nodes to be provided on top of SSC/Hosting Appliance architecture.
...
## KYOK
The abbreviation of Keep Your Own Key, which allows you to configure the master key to take the full control of the cloud HSM, so that you have full control and authority over encryption keys that you can bring, control, and manage.
...
## Namespace
A namespace such as ibmzcontainersthat contains a number of unique images. For examples, the images include hpvsop-base, hyperpcons-worker, hyperpcons-riaas, and so on.

## Node
Member of a Hyper Protect DBaaS cluster. Nodes in one cluster are runq containers/VMs that run on shared hardware. The equivalent of server in PostgreSQL terms.
...
## Primary node
The cluster member that receives all write operations in Hyper Protect DBaaS.
...
## Replica
The secondary nodes in a Hyper Protect DBaaS cluster. See "Secondary node".
...
## Replication lag
The length of time between the last operation in the primary’s oplog and the last operation applied to a particular secondary.

## RIAS
The abbreviation of Regional Infrastructure API Service, which offers the APIs associated with providing the VPC User experience for Cloud IaaS. Genesis and control plane sit behind the RIAS API layer.
...
## VPC 
The abbreviation of Virtual Private Cloud, which is the basis of IBM Cloud IaaS offerings into the future. The User experience is on top of Genesis.

## VPC Gen 1
Early implementation of VPC on top of pre-existing APIs rather than Genesis control plane.

## VPC Gen 2
The version of IaaS which provides VPC on top of the pure genesis control plane. This is the first entry point for the ZaaS IaaS.
